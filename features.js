...
async function loadMyChallengeTeams() {
    ...
    teams.forEach(team => { const o = document.createElement("option");
        o.value = team.id;
...
async function loadOpponentTeams() {
    ...
    opponents.forEach(team => { const o = document.createElement("option");
        o.value = team.id;
        o.textContent = team.name;
        select.appendChild(o); });
}
...
async function sendChallenge() {
    ...
    if (selectedChallengeMode === "team") {
        ...
        if (teamError) { await client.from("challenges").delete().eq("id", challenge.id);
            message.textContent = "Team error: " + teamError.message; return; }
    }
...
async function recordChallengeResult(challengeId) {
    ...
    modal.querySelectorAll('.result-btn-option').forEach(btn => {
        btn.addEventListener('click', function() {
            modal.querySelectorAll('.result-btn-option').forEach(b => { b.style.opacity = '0.5';
    ...
    modal.querySelectorAll('.score-btn-my').forEach(btn => {
        btn.addEventListener('click', function() {
            modal.querySelectorAll('.score-btn-my').forEach(b => { b.style.opacity = '0.5';
                b.style.transform = 'scale(0.95)'; });
            this.style.opacity = '1';
            this.style.transform = 'scale(1)';
    ...
    modal.querySelectorAll('.score-btn-opp').forEach(btn => {
        btn.addEventListener('click', function() {
            modal.querySelectorAll('.score-btn-opp').forEach(b => { b.style.opacity = '0.5';
                b.style.transform = 'scale(0.95)'; });
            this.style.opacity = '1';
            this.style.transform = 'scale(1)';
    ...
    function checkResultAllSelected() {
        ...
        if (selectedResult && myScore !== null && opponentScore !== null) {
            ...
            confirmBtn.textContent = '⚠️ SELECT ALL';
    ...
    document.getElementById('confirmResultBtn').addEventListener('click', async function() {
        ...
        const rules = { ...(c.rules || {}),
            result: selectedResult === 'draw' ? 'draw' : 'win',
            winner_id: winnerId,
            my_score: myScore,
            opponent_score: opponentScore,
            recorded_by: user.id,
            recorded_at: new Date().toISOString(),
            winner_name: selectedResult === 'them' ? (other?.full_name || 'Opponent') : null };
...
async function loadInterestsEditor() {
    ...
    interestSkillCache = skills;
    currentInterestIds = new Set((parts || []).map(x => x.subcategory_id));
    const groups = new Map();
    skills.forEach(s => { const cat = s.categories?.name || "Other"; if (!groups.has(cat)) groups.set(cat, []);
...
async function loadMyQuickStats() {
    const box = document.getElementById("myQuickStats");
    if (!box) return;
    const { data: { user } } = await client.auth.getUser();
    if (!user) return;
...
async function loadLeaderboard(filter) {
    ...
    const btnId = filter === 'overall' ? 'lbOverall' :
        filter === 'bca' ? 'lbBCA' :
        filter === 'bba' ? 'lbBBA' : 'lbBCOM';
    ...
    challenges?.forEach(c => {
        ...
        if (rules.winner_id) {
            if (rules.winner_id === c.challenger_id && challenger) {
                ...
                if (challenger) { challenger.losses++;
                    challenger.total++; }
            }
        } else if (rules.result === 'draw') {
            if (challenger) { challenger.draws++;
                challenger.total++;
                challenger.points += 1; }
    ...
    Object.values(userStats).forEach(u => {
        ...
        u.rank = rankInfo.rank;
        u.rankLabel = rankInfo.label;
        u.rankColor = rankInfo.color;
        u.rankEmoji = rankInfo.emoji;
    });
    let filtered = Object.values(userStats);
    if (filter === 'bca') { filtered = filtered.filter(u => u.course === 'BCA'); } else if (filter === 'bba') { filtered = filtered.filter(u => u.course === 'BBA'); } else if (filter === 'bcom') { filtered = filtered.filter(u => u.course === 'B.COM'); }
...
async function loadCategoryRankings(category) {
    ...
    challenges.forEach(c => {
        ...
        const opponentId = c.opponent_id;
        const rules = c.rules || {};
        const subcategory = c.subcategories;
        const categoryName = subcategory?.categories?.name || 'Unknown';
        const gameName = subcategory?.name || 'Unknown';
        ...
        if (rules.winner_id) {
            const winnerId = rules.winner_id;
            ...
            userStats[winnerId].total++;
            userStats[winnerId].games[gameName].wins++;
            userStats[winnerId].games[gameName].total++;
            ...
            userStats[loserId].losses++;
            userStats[loserId].total++;
            userStats[loserId].games[gameName].losses++;
            userStats[loserId].games[gameName].total++;
            ...
            [challengerId, opponentId].forEach(id => {
                ...
                userStats[id].draws++;
                userStats[id].total++;
                userStats[id].games[gameName].draws++;
                userStats[id].games[gameName].total++;
            });
        }
    });
    Object.keys(userStats).forEach(userId => {
        ...
        Object.keys(stats.games).forEach(gameName => {
            ...
            game.rank = rankInfo.rank;
            game.rankLabel = rankInfo.label;
            game.rankColor = rankInfo.color;
            game.rankEmoji = rankInfo.emoji;
        });
    });
    let html = '';
        ...
        games.forEach(gameName => {
            ...
            if (topPlayers.length === 0) {
                ...
                topPlayers.forEach((playerId, index) => {
                    ...
                    let bgColor = '#0a0a0a';
                    if (index === 0) { medal = '🥇';
                        bgColor = '#1a1508'; } else if (index === 1) { medal = '🥈';
                        bgColor = '#151515'; } else if (index === 2) { medal = '🥉';
                        bgColor = '#1a0f0a'; }
...
async function loadTournaments(filter) {
    ...
    if (user) {
        ...
        if (adminControls) {
            adminControls.style.display = (currentUserRole === 'super_admin' || currentUserRole === 'college_admin') ? 'block' : 'none';
        }
    }
...
function closeCreateTournament() { document.getElementById("createTournamentModal").style.display = "none";
    document.getElementById("tournMessage").textContent = ""; }
...
async function createTournament() {
    ...
    if (!title || !game || !startDate || !endDate) { msg.textContent = "Please fill in all required fields.";
        msg.style.color = "#ff6b6b"; return; }
    if (new Date(endDate) <= new Date(startDate)) { msg.textContent = "End time must be after start time.";
        msg.style.color = "#ff6b6b"; return; }
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = "Please login first.";
        msg.style.color = "#ff6b6b"; return; }
    ...
    if (error) { msg.textContent = "Error: " + error.message;
        msg.style.color = "#ff6b6b"; return; }
    msg.textContent = "✅ Tournament created successfully!";
    ...
    setTimeout(() => { closeCreateTournament();
        loadTournaments('all'); }, 1500);
}
...
function displayGames(category) {
    ...
    filtered.forEach(game => {
        ...
        card.onmouseenter = () => { card.style.borderColor = '#555';
            card.style.transform = 'translateY(-3px)'; };
        card.onmouseleave = () => { card.style.borderColor = '#292929';
...
function filterGames(category) { currentGameFilter = category;
...
function showGameLaunchNotification(game) {
    ...
    setTimeout(() => { notif.style.animation = 'slideOutDown 0.3s ease';
...
function showAddGameModal() { document.getElementById("addGameModal").style.display = "flex";
    document.getElementById("gameModalMessage").textContent = ""; }
...
async function saveGame() {
    ...
    if (!name) { msg.textContent = "Please enter a game name.";
        msg.style.color = "#ff6b6b"; return; }
    if (!url) { msg.textContent = "Please enter a launch URL.";
        msg.style.color = "#ff6b6b"; return; }
    msg.textContent = "Saving game...";
    msg.style.color = "#888";
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = "Please login first.";
        msg.style.color = "#ff6b6b"; return; }
    ...
    if (error) { if (error.code === '23505') { msg.textContent = "⚠️ This game already exists!";
            msg.style.color = "#ff6b6b"; } else { msg.textContent = "Error: " + error.message;
            msg.style.color = "#ff6b6b"; } return; }
    msg.textContent = "✅ Game added successfully!";
    msg.style.color = "#00b894";
    setTimeout(() => { closeAddGameModal();
        loadGames(); }, 1500);
}
...
async function challengeQuizOpponent(opponentId) {
    ...
    const { data: challenge, error } = await client.from("challenges").insert({
        ...
        subcategory_id: null,
        title: '📚 Quiz Battle',
        description: `Quiz Battle on ${selectedCategory === 'all' ? 'General' : selectedCategory}`,
...
async function startQuizBattle() {
    ...
    if (!questions || questions.length === 0) { alert("No questions generated. Please try again!");
        closeQuizLoading(); return; }
    currentQuestions = questions;
    currentQuestionIndex = 0;
    playerScore = 0;
    opponentScore = 0;
...
function startQuizTimer() {
    ...
    quizTimer = setInterval(() => {
        timeLeft--;
        if (timeLeft <= 0) { clearInterval(quizTimer);
            handleQuizAnswer(-1); }
    }, 1000);
}
...
function endQuizBattle() {
    ...
    let winnerText = '',
        winnerColor = '#fdcb6e';
    if (playerScore > opponentScore) { winnerText = '🎉 You Won!';
        winnerColor = '#00b894'; } else if (playerScore < opponentScore) { winnerText = '💪 Opponent Won!';
        winnerColor = '#e17055'; } else { winnerText = '🤝 It\'s a Draw!';
        winnerColor = '#fdcb6e'; }
...
async function addQuizQuestion() {
    ...
    msg.textContent = "Adding question...";
    msg.style.color = "#888";
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = "Please login first.";
        msg.style.color = "#ff6b6b"; return; }
    ...
    if (profile?.role !== 'super_admin' && profile?.role !== 'college_admin') {
        msg.textContent = "⚠️ Only admins can add questions!";
        msg.style.color = "#ff6b6b";
        return;
    }
...
async function addSubject() {
    ...
    if (!name) { msg.textContent = '⚠️ Please enter a subject name.';
        msg.style.color = '#ff6b6b'; return; }
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = 'Please login first.';
        msg.style.color = '#ff6b6b'; return; }
    const { data: existing } = await client.from('student_subjects').select('id').eq('student_id', user.id).eq('subject_name', name).maybeSingle();
    if (existing) { msg.textContent = '⚠️ You already have this subject!';
        msg.style.color = '#ff6b6b'; return; }
    msg.textContent = 'Adding subject...';
    msg.style.color = '#888';
...
async function addGrade() {
    ...
    if (!subjectId) { msg.textContent = '⚠️ Please select a subject.';
        msg.style.color = '#ff6b6b'; return; }
    if (!examName) { msg.textContent = '⚠️ Please enter an exam name.';
        msg.style.color = '#ff6b6b'; return; }
    if (isNaN(marksObtained)) { msg.textContent = '⚠️ Please enter valid marks.';
        msg.style.color = '#ff6b6b'; return; }
    if (marksObtained > totalMarks) { msg.textContent = '⚠️ Marks cannot exceed total marks.';
        msg.style.color = '#ff6b6b'; return; }
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = 'Please login first.';
        msg.style.color = '#ff6b6b'; return; }
    msg.textContent = 'Adding grade...';
    msg.style.color = '#888';
    ...
    if (error) { msg.textContent = 'Error: ' + error.message;
        msg.style.color = '#ff6b6b'; return; }
    msg.textContent = '✅ Grade added successfully! 🎉';
    msg.style.color = '#00b894';
...
async function addCollege() {
    ...
    if (!name || !code) { message.textContent = "Name and code required.";
        return; }
...
async function addCollegeAdmin() {
    ...
    if (!userId) { message.textContent = "Enter user ID.";
        return; }
    ...
    if (!profile) { message.textContent = "User not found.";
        return; }
    ...
    await loadAdmins();
    await loadCollegeManagementCounts(selectedCollegeId);
}
...
function displayStudentsByCategory(category) {
    ...
    sortedCourses.forEach(course => {
        const years = grouped[course];
        const yearKeys = Object.keys(years).sort((a, b) => { const numA = parseInt(a);
            const numB = parseInt(b); return numA - numB; });
        let courseColor = '#6c5ce7';
        if (course === 'BCA') courseColor = '#6c5ce7';
        else if (course === 'BBA') courseColor = '#00b894';
        else if (course === 'B.COM') courseColor = '#fdcb6e';
...
async function loadCollegeAdminDashboard(user, profile) {
    ...
    if (error || !admin) { alert("No college assigned.");
        showAuth(); return; }
    showOnlyAdmin();
...
async function createTeam() {
    ...
    document.getElementById('confirmTeamBtn').addEventListener('click', async function() {
        ...
        if (!name) { msg.textContent = '⚠️ Please enter a team name';
            msg.style.color = '#ff6b6b'; return; }
        ...
        if (existing && existing.length > 0) { msg.textContent = '⚠️ You\'re already in a team!';
            msg.style.color = '#ff6b6b';
            this.textContent = '✅ CREATE';
            this.disabled = false; return; }
        ...
        setTimeout(() => { document.body.removeChild(overlay);
...
async function sendTeamInvite(teamId) {
    ...
    if (!inviteeId) { msg.textContent = "Select a player.";
        return; }
    const { data: { user } } = await client.auth.getUser();
    if (!user) { msg.textContent = "Please login.";
        return; }
    ...
    if (existing?.length) { msg.textContent = "Invite already sent.";
        return; }
